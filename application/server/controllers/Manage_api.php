...
public function getChat()
	...
	public function myMentors()
	{
		...
		if($mentCount > 0){
			$mentors = $this->db->query('SELECT a.*, b.*,c.* FROM tb_current_course AS a JOIN tb_mentors AS b ON a.mentor=b.mentor_id RIGHT JOIN tb_subject AS c ON a.subject=c.subject_id WHERE a.user_id='.$userId)->result();
	...
	public function mentorList()
	{
		...
		// print_r($uniqueMentors);
			...
			header('Access-Control-Allow-Origin: *');
			echo json_encode(array('success' => 1,'mentlist'=>$mentquerylist));
		}
		else{
			header('Access-Control-Allow-Origin: *');
			echo json_encode(array('success' => 0));
